...
### 2. 
##### 소득이 높을수록 신용카드 사용량이 많아진다고 하는데, 월 소득 대비 신용카드 사용량을 조사하였더니 아래와 같이 나왔다고 한다. 이 자료를 바탕으로 해서 회귀식을 구하고, 월 소득이 250만원일 때 신용카드 사용량을 예측하시오. 
...
###### * 참조 <br> Modelling Home Prices (R) by Campos, David and Eulogio, Raul <br> 모범답안 by 황성윤
```{r}
...
######rad 데이터는 방사성 고속도로로의 접근성을 index 화 한 것. 이는 접근성에 따라 ordered 되어있으므로 level 을 더한다.
```{r}
...
plot(subset2)
```
###### BIC 그래프 모형에 따르면 crim 과 tax 가 무의미하게 판정된다. 따라서 앞으로의 검정에서 crim, tax, age, indus, rad 를 제외시킨다.


#### II. 기타 model
###### 다중선형회귀 모델 구축시 잔차에 대한 다음과 같은 가정을 한다. <br>정규성 (normality)<br>등분산성 (homoscedaticity)<br>선형성 (linearity)<br>독립성 (idependence)
...
###### 2. 등분산성 및 선형성<br>gvlma 패키지의 gvlma 함수가 등분산성과 선형성을 검정한다. global test : 선형성, heteroscedasticity : 등분산성
```{r}
library(gvlma)
(gvmodel <- gvlma(fit2))
```
...
###### 3. 독립성 
###### Durbin-Watson 검정법으로 독립성을 검정한다.이 때는 DW statistic == 2 일 때 독립, ==0 일때 양의 자기상관, ==4 일때 음의 자기상관으로 판정한다.
```{r}
library(car)
...
##### 결과적으로 fit2 는 다중선형회귀분석에 대한 기본가정을 만족하지 못한다.그러나 현 과제 특성상 이 모델로 우선 진행한다.

<br><br>

###### 다중공선성 multicollinearity 를 검정하기 위해 vif 테스트를 시행한다. vif 값이 5보다 높을 경우 문제가 있는 것으로 판단한다.
```{r}
vif(fit2) 
```
...
###### zn 은 무의미한 변수로 판정되고, chas 도 약한 연관성을 보이나 나머지 변수들은 매우 유의미하게 판정된다.
```{r}
par(mfrow=c(2,2))
plot(fit3)  
```
...
##### 따라서 fit4 model 이 현재 가장 유력한 모델으로, 즉 crim, tax, age, indus, rad 를 제외하고 lstat^2 의 독립변수가 종속 변수 medv 와 유의미한 관계가 있는 것으로 판단하고boston 의 집값 medv를 예측하기 위한 다중회귀식의 판정을 멈추도록 한다.
